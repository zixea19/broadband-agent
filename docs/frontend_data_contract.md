...
<!--event:plan-->                  ← 阶段规划（含所有 Phase 概览）
  ↓
for each Phase:
  ├─ <!--event:decompose_result--> ← Phase 步骤分解摘要（新增）
  ...
  ↓
[ToolCall] render_report.py        ← Markdown 报告（通过 ToolCallCompleted stdout 展示）
<!--event:done-->                  ← 流程结束信号
summary JSON 代码块                ← 结构化交接契约（供 Orchestrator 消费）
```

> **已移除**：`<!--event:report-->` — 其数据与 stdout + step_result 完全重复，前端未实现渲染器。由 `render_report.py` stdout + `<!--event:done-->` 替代。
...
```
**前端渲染**：步骤分解摘要表，让用户在 Execute 开始前了解即将执行的步骤。
...
| 字段 | 说明 |
|---|---|
| `status` | `"ok"` 正常 / `"error"` Step 执行失败 |
| `phase_id` + `step_id` | 唯一标识，用于关联 ToolCallCompleted stdout |

**前端渲染**：步骤卡片（标题=insight_type，正文=summary，标签=significance）。
**通道关联**：完整的 `chart_configs` 和 `filter_data` 在对应的 ToolCallCompleted 事件的脚本 stdout 里（`op: "run_insight"`），stdout JSON 中现已包含 `phase_id` + `step_id` 字段，前端可通过这对 ID 精确关联。
...
> `<!--event:report-->` 已从协议中移除。其数据与 `render_report.py` stdout + 各阶段 `step_result` 完全重复，
> 且前端（`chat_renderer.py`）未实现该事件的渲染器。最终报告改为通过：
> - `render_report.py` stdout (ToolCallCompleted) — Markdown 报告
> - `<!--event:done-->` — 流程结束信号
> - summary JSON 代码块 — 结构化交接契约
...
| `chart_configs` | dict | **完整的 ECharts option JSON 数据**，当前以折叠块展示（Gradio Chatbot 不原生支持 ECharts 渲染） |
| `found_entities` | dict | 下钻实体，如 `{"portUuid": [...]}`，可作为关联标签展示 |
| `data_shape` | [int, int] | 查询结果的完整行列数（filter_data 是截断后的） |
| `phase_id` | int \| null | 由 InsightAgent 传入，用于关联 assistant 文本中的 `step_result` 事件 |
| `step_id` | int \| null | 同上 |
...
{
  "status": "error",
  "skill": "<对应 skill 名, 如 insight_query / insight_nl2code / insight_decompose>",
  "op": "<对应 op, 如 run_insight / run_nl2code / list_schema>",
  "error": "错误描述信息"
}
```
