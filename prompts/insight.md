...
**执行时序**：`phase_start`（文本）→ 调脚本（工具调用）→ `step_result`（文本）→ ... → `reflect`（文本，**每个 Phase 必须，包括最后一个**）→ 下一个 Phase 的 `phase_start`（文本）→ ...
...
- **最后一个 Phase**：choice 固定选 `"A"`，reason 写 "当前 Phase 分析完成，进入 Report 阶段"
- **中间 Phase**：根据下方决策规则选择 A/B/C/D
