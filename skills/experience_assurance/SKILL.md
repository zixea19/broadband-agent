...
| 字段                            | 类型 | 必填   | 默认值 | 允许值 | 说明            |
|-------------------------------|---|------|--|---|---------------|
| `application-type`            | string | 条件必填 | anchor-video | anchor-video/real-time-game/cloud-platform/online-office | 应用类型          |
| `application`                 | string | 条件必填 | TikTok | TikTok/Kwai | 保障应用        |
| `business-type`               | string | 是    | experience-assurance | experience-assurance/speed-limit/app-flow | 策略配置文件名称      |

注意: 只有业务类型为experience-assurance的时候需要指定application-type和application
