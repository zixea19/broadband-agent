...
| 方案字段值 | CLI 值 | 说明 |
|---|---|---|
| experience-assurance | `experience-assurance` | 体验保障（wifi 时隙切片，需指定 application-type/application） |
| speed-limit | `speed-limit` | 限速（PON 口大象流检测 + 限速，不需要 application 相关参数） |
| app-flow | `app-flow` | APP Flow（PON 口切片，独占保障，不需要 application 相关参数） |

**场景 → 业务类型选择**（plan_design SKILL.md 已含此规则，仅作备查）：

| 方案场景 | business-type |
|---|---|
| 单用户应用切片（wifi 连接场景） | `experience-assurance` |
| PON 口大象流整形 | `speed-limit` |
| 高保障 VIP / PON 口独占 | `app-flow` |
