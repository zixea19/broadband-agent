...
2. 根据 `compare` 开关分派 basic / compare 模式到 `home_wifi_engine`；
...
契约详见 `docs/wifi_simulation_redesign.md` §3.4。
"""
...
def _validate(params: dict[str, Any]) -> list[str]:
    ...
    if params.get("compare"):
        ...
        if not isinstance(target, int) or target <= ap:
            errors.append(f"compare 模式要求 target_ap_count 为 > ap_count 的整数，当前={target!r}")
    return errors
...
def _run(params: dict[str, Any]) -> dict[str, Any]:
    ...
    compare = bool(params["compare"])
    target_ap = params["target_ap_count"] if compare else None
    ...
    if compare:
        # ── compare 模式：2 对比 PNG + 4 JSON + 4 NPY（NPY 不曝光） ─────────────
        ...
        # ── basic 模式：2 独立 PNG，无 JSON 数据文件 ─────────────────────────────
        rssi_png = out_dir / f"{preset}_rssi_ap{ap}.png"
        stall_png = out_dir / f"{preset}_stall_ap{ap}.png"
        _engine.generate_rssi_heatmap(preset, ap, str(rssi_png), grid_size=grid)
        _engine.generate_stall_grid(preset, ap, str(stall_png), grid_size=grid)
        image_paths = [
            {"label": f"{preset} RSSI 热力图", "path": str(rssi_png), "kind": "rssi"},
            {"label": f"{preset} 卡顿率栅格图", "path": str(stall_png), "kind": "stall"},
        ]

    summary = _build_summary(preset, ap, target_ap, compare, stats)
    out: dict[str, Any] = {
        "skill": "wifi_simulation",
        "status": "ok",
        "mode": "compare" if compare else "basic",
        "preset": preset,
        "grid_size": grid,
        "ap_count": ap,
...
def _build_summary(
    ...
    if compare and stats:
        ...
        return "；".join(segs)
    return f"{preset} {ap}AP 信号仿真完成，已产出 RSSI 热力图与卡顿率栅格图"
